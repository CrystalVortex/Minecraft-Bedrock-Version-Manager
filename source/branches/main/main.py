...
from dependencies.lib.downloader.bedrock.versions import Versions # for getting links to download an appx

import tkinter # gui window thing

from dependencies.lib import log4py # a custom made logging module, find it in dependencies/lib/log4py.py

import wget # Used to download file

import threading # create a thread to make sure wget.download() (for downloading files) does not freeze the gui/window
...
app.title("MCBE Version Launcher 7.3") 
...
def onetwenty():
    def r():
        log4py.INFO("Downloading 1.19.20...")
        version = Versions.get_by_version("1.20.0.1")
        log4py.SUCCESS(version.uri)
        wget.download(version.uri)
        log4py.INFO("\n")
        log4py.SUCCESS("Download complete")
...
button = customtkinter.CTkButton(master=app, text="Download 1.20.0", command=onetwenty)
button.place(relx=0.10, rely=0.2, anchor=tkinter.CENTER)
